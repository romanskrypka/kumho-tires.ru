...
**Текущее состояние:** Платформа содержит контент шинного бренда Kumho. Контент частично отделён от ядра (JSON-файлы, параметризация коллекций в settings.php), но `DataLoaderService` и `PageAction` ещё содержат хардкоды tires/news. Отсутствует система ролей и личных кабинетов. Нет связи с n8n, не создан Django с сервисами.
...
|------|-----|-----------|-----------|
| **0. Отделение контента от ядра** | Универсальный загрузчик коллекций, чистка хардкодов, `project.php`, версионирование | Платформа content-agnostic: замена контента = JSON + Twig + CSS, 0 правок PHP | Переиспользуемость, Гибкость |
| **0.5. Инфраструктура тиражирования** | Контракты, Event Dispatcher, реестр, scaffold, JSON-логи | Готовность к тиражированию | Тиражируемость, Гибкость |
| **1. Контент «Ритейл Логистик»** | Новые страницы, ребрендинг, удаление Kumho-контента | Публичный сайт РЛ | Тиражируемость, Индивидуальность |
...
### Шаг 7: Версионирование ядра

1. Создать `CHANGELOG.md` (формат Keep a Changelog)
...
`tools/scaffold/create-deployment.sh` — создаёт полную структуру `deployments/{client}/` включая `docker-compose.yml`, `.env`, `project.php`, дефолтные данные, конфиг n8n-workflow'ов.
...
| # | Проверка | Критерий |
|---|----------|---------|
| 1 | Ноль хардкодов контента в PHP | `grep -r "tires\|kumho\|Kumho" src/` = 0 |
| 2 | Новая коллекция без правки PHP | Добавить `test-items` в collections -> работает |
| 3 | Удаление коллекции без ошибок | Убрать `tires` -> остальные работают, 404 для /tires/ |
| 4 | Тесты проходят | `php vendor/bin/phpunit` = 0 failures |
| 5 | Статический анализ чист | `vendor/bin/phpstan analyse` = 0 errors |
| 6 | project.php отделён | Проектные настройки в `config/project.php`, не в `settings.php` |
| 7 | Дефолты при отсутствии project.php | Удалить project.php -> ядро загружается, главная работает |
| 8 | project.php.dist существует | Шаблон содержит все ключи с дефолтами |
| 9 | CHANGELOG.md создан | Содержит версию 1.0.0 |
| 10 | Тег v1.0.0 создан | `git tag` содержит `v1.0.0` |
...
| 2 | Event Dispatcher работает | Подписчик на `PageLoaded` вызывается при загрузке |
| 3 | Реестр заполнен | `docs/registry/components.md` содержит все компоненты |
| 4 | Scaffold работает | `create-deployment.sh test-client` создаёт рабочий проект |
| 5 | JSON-формат логов | `tail -1 logs/app.log` парсится как JSON |
| 6 | Duration в логах | Логи содержат `duration_ms` |

### Этап 1: Контент заказчика

| # | Проверка | Критерий |
|---|----------|---------|
| 1 | Развёрнуто по профилю | Через scaffold, не ручным копированием |
