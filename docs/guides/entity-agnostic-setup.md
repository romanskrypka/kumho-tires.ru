# [АРХИВ — выполнено в Этапе 0]

> **Статус:** Данная инструкция описывает ручную миграцию, которая была полностью выполнена в рамках Этапа 0 (v1.0.0). Ядро платформы (`src/`) теперь content-agnostic: универсальные методы `loadEntitySlugs`/`loadEntity`, единый цикл `foreach ($collections)` в `PageAction`, конфигурация коллекций в `config/project.php`. Для добавления новых коллекций используйте scaffold-генератор: `npm run create-collection <slug>`. Подробнее: [docs/session-2026-03-07-content-agnostic.md](../session-2026-03-07-content-agnostic.md).
